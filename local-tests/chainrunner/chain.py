...
class Chain:
    """Chain is a class for orchestrating setting up and interaction with a local aleph-node blockchain.
    ...
    def bootstrap(self, binary, accounts, **kwargs):
        ...
        with open(chainspec, 'w') as f:
            subprocess.run(cmd, stdout=f).check_returncode()
    ...
    def set_flags(self, *args, **kwargs):
        """Set common flags for all nodes. Positional arguments are used as binary flags and should be strings.
        Keyword arguments are translated to valued flags: `my_arg=some_val` results in `--my-arg some_val`
        in the binary call. Seq (type alias for int) can be used to specify numerical values that should
        be different for each node. `val=Seq(13)` results in `--val 13` for node0, `--val 14` for node1 and so on."""
        for k in args:
            for n in self.nodes:
                n.flags[k] = True
